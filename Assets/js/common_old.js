$(document).ready(function(e){
	...
	$('#ruang').autocomplete({
		source:'ajax.php?type=autocomplete', minLength:2	
	});
	$('input:checkbox#Lain').change(function(){
        if(this.checked){
			$("input:hidden#select2-1").removeAttr('disabled');
		} else {
			$("input:hidden#select2-1").removeAttr('enabled');
		}
	});
	$('#checkbox_kehadiran input:checkbox').change(function(){
		if(this.checked){
			var id = $(this).val();
